...
import { createEmbed, notLinkedEmbed, privacyEmbed, unexpectedErrorEmbed } from '../util/embed';
...
const RecentlyPlayingComponents = createPaginationComponentTypes('recently-playing');
...
export default createCommand(RecentlyStreamedCommand)
  .registerChatInput(async ({ interaction, args, statsfmUser: statsfmUserSelf, respond }) => {
    ...
    if (!privacySettingCheck) {
      ...
      return respond(interaction, {
        embeds: [
          privacyEmbed(
            targetUser,
            privacyManager.getPrivacySettingsMessage('recentlyPlayed', 'recentlyPlayed')
          )
        ]
      });
    }
    ...
    try {
      recentlyStreamed = await statsfmApi.users.recentlyStreamed(statsfmUser.id);
    } catch (err) {
    ...
    if (recentlyStreamed.length === 0) {
      ...
      return respond(interaction, {
        embeds: [
          createEmbed().setTitle(`${targetUser.username} has not streamed recently`).toJSON()
        ]
      });
    }
    ...
    const message = await respond(
      interaction,
      pagination.createMessage<'reply'>(await pagination.current(), RecentlyPlayingComponents)
    );

    pagination.manageCollector(message, RecentlyPlayingComponents, interaction.user);
