...
export class Config {
  ...
  constructor(@inject(kLogger) public readonly logger: Logger) {
    ...
    this.excludeFromCommandCooldown = process.env.COOLDOWN_EXCLUDE?.split(',') ?? [];
  ...
  private envCheck() {
    ...
    ow(process.env.STATSFM_HTTP_API_URL!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(
      process.env.STATSFM_HTTP_API_USER_AGENT_APPENDIX!,
      ow.any(ow.nullOrUndefined, ow.string.nonEmpty)
    );
    ow(process.env.STATSFM_HTTP_API_RETRIES!, ow.any(ow.nullOrUndefined, ow.number));
    ow(process.env.STATSFM_HTTP_API_VERSION!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(process.env.STATSFM_AUTH_ACCESS_TOKEN!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(process.env.ANALYTICS_TOKEN!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(process.env.ANALYTICS_URL!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(process.env.COOLDOWN_EXCLUDE!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
    ow(process.env.PRIVATE_API_TOKEN!, ow.any(ow.nullOrUndefined, ow.string.nonEmpty));
  }
}
